...
def build_sample_monthly_data(root: Path | None = None) -> None:
    ...
    for market_id, market_class, sensitivity in [
        ...
        returns = 0.55 + cycle + sensitivity * gpr_z
    ...
    gdelt = pd.DataFrame(
        {
            ...
            "conflict_count": (10 + np.maximum(gpr_z, 0) * 6).round().astype(int),
            "protest_count": (8 + np.maximum(np.sin(step / 5), 0) * 4).round().astype(int),
            "sanction_count": (3 + (step % 17 == 0) * 2).astype(int),
            "diplomatic_conflict_count": (5 + np.maximum(gpr_z, 0) * 3).round().astype(int),
            "avg_goldstein": (-1.5 - np.maximum(gpr_z, 0)).round(4),
            "avg_tone": (-2.0 - np.maximum(gpr_z, 0) * 0.8).round(4),
            "risk_index_raw": np.log1p(10 + np.maximum(gpr_z, 0) * 8).round(4),
